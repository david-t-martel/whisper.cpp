...
// mimalloc memory allocator is available
#endif

#ifdef WHISPER_USE_TBB
// Intel TBB parallel processing is available
#endif

#ifdef WHISPER_USE_BOOST_STACKTRACE
// Boost stacktrace support is available
#endif

#ifdef WHISPER_USE_CUDA_API_WRAPPERS
// Enhanced CUDA error handling is available
#endif

#ifdef WHISPER_USE_TASKFLOW
// Taskflow parallel processing is available
#endif
...
extern "C"
{
...
#ifdef WHISPER_USE_CUDA_API_WRAPPERS
        bool cuda_enhanced_errors; // Enable enhanced CUDA error reporting
        int cuda_memory_pool_size; // CUDA memory pool size in MB (0 = default)
#endif

#ifdef WHISPER_USE_MIMALLOC
        bool use_mimalloc; // Use mimalloc allocator
#endif

#if defined(WHISPER_USE_TBB) || defined(WHISPER_USE_TASKFLOW)
        int parallel_strategy; // 0 = default, 1 = TBB, 2 = Taskflow
        int parallel_workers;  // Number of parallel workers (0 = auto)
#endif
    };
    ...
    struct whisper_full_params
    {
        ...
        // New parameters for extended libraries
#ifdef WHISPER_USE_MIMALLOC
        bool use_mimalloc_arena; // Use mimalloc arena allocator for temporary allocations
#endif

#ifdef WHISPER_USE_BOOST_STACKTRACE
        bool capture_stacktrace; // Capture stacktrace on errors
#endif

        // Existing grammar fields
...
#ifdef WHISPER_USE_BOOST_STACKTRACE
    // Get detailed stacktrace information for the last error
    WHISPER_API const char *whisper_get_stacktrace(void);

    // Set custom error handler
    WHISPER_API void whisper_set_error_handler(void (*handler)(const char *error, const char *stacktrace));
#endif

#ifdef WHISPER_USE_MIMALLOC
    // Get information about memory usage
    WHISPER_API size_t whisper_get_memory_usage(void);

    // Force garbage collection of unused memory
    WHISPER_API void whisper_collect_garbage(void);
#endif

#if defined(WHISPER_USE_TBB) || defined(WHISPER_USE_TASKFLOW)
    // Set parallel processing parameters
    WHISPER_API void whisper_set_parallel_params(struct whisper_context *ctx, int strategy, int n_workers);

    // Check if parallel processing is available
    WHISPER_API bool whisper_has_parallel_support(struct whisper_context *ctx);
#endif

#ifdef WHISPER_USE_CUDA_API_WRAPPERS
    // Get detailed CUDA error information
    WHISPER_API const char *whisper_get_cuda_error_info(void);

    // Set CUDA memory pool parameters
    WHISPER_API bool whisper_set_cuda_memory_pool(struct whisper_context *ctx, int size_mb);
#endif

// Add parallel implementation of PCM to MEL conversion
#if defined(WHISPER_USE_TBB) || defined(WHISPER_USE_TASKFLOW)
    // Parallel implementation of PCM to MEL conversion
    WHISPER_API int whisper_pcm_to_mel_parallel(
        struct whisper_context *ctx,
        const float *samples,
        int n_samples,
        int parallel_strategy, // 0 = default, 1 = TBB, 2 = Taskflow
    ...
    WHISPER_API int whisper_pcm_to_mel_parallel_with_state(
        struct whisper_context *ctx,
        struct whisper_state *state,
        const float *samples,
        int n_samples,
